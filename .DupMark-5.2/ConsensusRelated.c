...
unsigned int DupMarkWithOutUMI(time_t start,unsigned int MapReadsNum,unsigned int *GroupInfo4ReadsID,unsigned int *SortInfo4MapFull,unsigned int *GroupInfo4MapFull,unsigned char *Info4Index,unsigned int *Info4BQ,unsigned int BamGenJumpFlag,unsigned char *DupLogPath)
{
	...
	while(From < MapReadsNum)
	{
		...
		if(!BamGenJumpFlag)
		{
			...
			for(i = 0;i < PairNum;i ++)
			{
				tBQ = *(Info4BQ + *(GroupInfo4ReadsID + *(SortInfo4MapFull + Read1[i])));
				if(tBQ > MaxBQ)
				{
					MaxId = i;
					MaxBQ = tBQ;
...
unsigned int DupMarkWithUMI(time_t start,unsigned char *SamFile,unsigned int MapReadsNum,unsigned long *Info4Shift,unsigned int *GroupInfo4ReadsID,unsigned int *SortInfo4MapFull,unsigned int *GroupInfo4MapFull,unsigned char *Info4Index,unsigned char *DupLogPath,unsigned char *UmiName,unsigned int HUmiSize,unsigned int FakeFlag)
{
	...
	while(From < MapReadsNum)
	{
		...
		// 按UMI序列进行排序；
		ReSortByUMI(PairNum,PairFrom,PairTo,Read1,Read2,UMISeq,HUmiSize,FRFlag);
		...
		else
		{
			...
			while(UMIFrom < PairNum)
			{
				...
				{
					if(! UMISeqCompare(*(UMISeq + UMITo),*(UMISeq + UMIFrom),HUmiSize))
					{
						break;
					}
				}
...
unsigned int DupCountWithUMI(time_t start,unsigned int MapReadsNum,unsigned int *GroupInfo4ReadsID,unsigned int *SortInfo4MapFull,unsigned int *GroupInfo4MapFull,unsigned char *Info4Index,unsigned long *Info4UMISeq,unsigned char *DupLogPath,unsigned int HUmiSize,unsigned int FakeFlag)
{
	FILE *fdlp;
	// 注意unmap的数量可能会超过这个数值;
	unsigned char FRFlag[100000],cmd[1000],LogBuffContent[100000];
	unsigned int PairFrom[100000],PairTo[100000],Read1[100000],Read2[100000];
	unsigned int tBQ,MaxBQ,From,To,UMIFrom,UMITo,PairNum,MaxId,i,j,AccumCheckPoint,AccumCheckMulti,CheckNum,FRNum,RFNum;
	unsigned int MaxLogBuffSize = 80000;
	unsigned int CurrLogBuffSize = 0;
	...
	while(From < MapReadsNum)
	{
		...
		// 按ReadsID顺序对SortInfo4MapFull重新进行局部排序，GroupInfo4ReadsID保持不变但SortInfo4MapFull变了；
		ReSortByString(From,To,SortInfo4MapFull,GroupInfo4ReadsID);
		// 假如都是un-map；
		if(*(GroupInfo4MapFull + *(SortInfo4MapFull + From)) == 0)
		{
		...
		for(i = 0;i < PairNum;i ++)
		{
			UMISeq[i] = *(Info4UMISeq + *(GroupInfo4ReadsID + *(SortInfo4MapFull + Read1[i])));
			FRFlag[i] = 0;
			UMISeq[i] &= 0x7fffffffffffffff;
		}
		// 按UMI序列进行排序；
